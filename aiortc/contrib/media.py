...
class MediaBlackhole:
    ...
    async def start(self):
        ...
        Start recording.
        """
        assert len(self.__tracks.items()) == 2
        for track, context in self.__tracks.items():
            if context.task is None:
                context.task = asyncio.ensure_future(self.__run_track_s1k2(track, context))
...
class MediaRecorder:
    ...
    def __init__(self, file, format=None, options={}):
        ...
        self.audio_resampler = av.AudioResampler(
            ...
            layout="mono",
            rate=16000,
            frame_size=int(16000 * AUDIO_PTIME)
        )
    ...
    def addTrack(self, track):
        ...
        if track.kind == "audio":
            ...
            stream.rate = 16000  # Set sampling rate to 16000 Hz
            stream.layout = "mono"  # Set channel layout to mono
        else:
    ...
    async def __run_track_s1k2(self, track: MediaStreamTrack, context: MediaRecorderContext):
        while True:
            try:
                frame = await track.recv()
            except MediaStreamError:
            ...
            if isinstance(frame, AudioFrame):
                ...
                if not resampled_frames:
                    continue
                data = resampled_frames[0].to_ndarray()

                self.custom_audio_process(audioframe=data[0])

                # save audio using av container
                #for packet in context.stream.encode(frame):
                #    self.__container.mux(packet)
            
    def custom_video_process(self, videoframe):
        pass
    
    def custom_audio_process(self, audioframe):
        pass
